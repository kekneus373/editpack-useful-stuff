...
for i in *."$extin"; do
    ...
    if [ -n "$aspect_ratio" ] && [ "$(echo "$aspect_ratio >= 1.76 && $aspect_ratio <= 1.78" | bc)" -eq 1 ]; then
        ffmpeg -i "$i" -c:v libxvid -b:v "$bv" -s 720x576 -aspect 16:9 -r 25 -c:a mp3 -b:a 192k divx/"${i%.*}.avi" # divx
    fi
done

echo "Processing complete. PRO TIP: Use 'ffmpeg-divx-file-16-9.sh' to process only one video file!"
