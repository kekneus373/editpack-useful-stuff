...
for i in *."$extin"; do
    ...
    # Check if aspect ratio is not approximately 16:9 (1.777...)
    if [ -n "$aspect_ratio" ] && [ "$(echo "$aspect_ratio < 1.76 || $aspect_ratio > 1.78" | bc)" -eq 1 ]; then
        ffmpeg -i "$i" -c:v libx264 -b:v "$bv1" -s 720x1280 -aspect 9:16 -preset veryfast -r 25 -c:a mp3 -b:a 128k compressed/"${i%.*}.mp4" # compress
    fi
done
...
for i in *."$extin"; do
    ...
    # Check if aspect ratio is not approximately 16:9 (1.777...)
    if [ -n "$aspect_ratio" ] && [ "$(echo "$aspect_ratio < 1.76 || $aspect_ratio > 1.78" | bc)" -eq 1 ]; then
        ffmpeg -i "$i" -c:v libxvid -b:v "$bv2" -s 576x720 -aspect 9:16 -r 25 -c:a mp3 -b:a 192k divx/"${i%.*}.avi" # divx
    fi
done

echo "Processing complete. PRO TIP: Use 'ffmpeg-compress-divx-file-<aspect ration>.sh' to process only one video file!"
