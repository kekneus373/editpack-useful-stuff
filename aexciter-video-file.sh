...
ffmpeg -i "$input_file" -af "aexciter=level_in=14.464:level_out=1.984:amount=1:drive=8.5:blend=0:freq=7500:ceil=9999" -c:v copy -c:a "$c" -b:a "$ba" -ar "$ar" "${input_file%.*}-a${input_file##*.}"
